...
DATA_DIR = Path("data")
DETAIL_FILE = DATA_DIR / "voix_detail.csv"

COMPONENTS = {
    "Voix TTC": "Voix_TTC",
    "SMS TTC": "SMS_TTC",
    "Djiguiya Voix&SMS TTC": "Djiguiya_Voix_SMS_TTC",
    "Sewa TTC": "Sewa_TTC",
    "Netaa Voix TTC": "Netaa_Voix_TTC",
    "Forfaits Voix Inter TTC": "Forfaits_Voix_Inter_TTC",
    "PAYG National TTC": "PAYG_National_TTC",
}
...
def load_data():
    if not DETAIL_FILE.exists():
        raise FileNotFoundError(
            "Le fichier data/voix_detail.csv est introuvable. "
    ...
    if missing:
        raise ValueError(f"Colonnes manquantes dans voix_detail.csv : {missing}")

    for col in COMPONENTS.values():
        detail[col] = pd.to_numeric(detail[col], errors="coerce").fillna(0)

    detail["Voix_SMS_Total_TTC"] = detail[list(COMPONENTS.values())].sum(axis=1)
    detail["Voix_SMS_Total_HT"] = detail["Voix_SMS_Total_TTC"] / 1.18

    summary = detail[["Periode", "Date", "Voix_SMS_Total_HT", "Voix_SMS_Total_TTC"]].copy()
...
def build_excel_download(summary_df, detail_df, summary_colors, detail_colors):
    ...
    ws1 = wb.active
    ws1.title = "Synthese"
    style_sheet(ws1, summary_df, summary_colors, title="VOIX & SMS - SYNTHESE")

    ws2 = wb.create_sheet("Detail")
    style_sheet(ws2, detail_df, detail_colors, title="VOIX & SMS - DETAIL")
...
def render():
    ...
    try:
        ...
        st.error(f"Impossible de charger les données Voix : {e}")
        return
    ...
    total_ht = summary_f["Voix_SMS_Total_HT"].sum()
    total_ttc = summary_f["Voix_SMS_Total_TTC"].sum()
    moyenne_journaliere = summary_f["Voix_SMS_Total_TTC"].mean()

    c1, c2, c3 = st.columns(3)
    c1.metric("Voix&SMS Total HT", format_montant(total_ht))
    c2.metric("Voix&SMS Total TTC", format_montant(total_ttc))
    c3.metric("Moyenne journalière TTC", format_montant(moyenne_journaliere))

    st.markdown("---")

    with st.expander("🔽 Détail des composantes Voix & SMS", expanded=True):
        pie_df = pd.DataFrame({
    ...
    summary_display = summary_display.rename(columns={
        "Voix_SMS_Total_HT": "Voix&SMS Total HT",
        "Voix_SMS_Total_TTC": "Voix&SMS Total TTC",
    })
    summary_display = summary_display[["Periode", "Voix&SMS Total HT", "Voix&SMS Total TTC"]]

    detail_display = detail_table_f.copy()
    detail_display["Periode"] = pd.to_datetime(detail_display["Periode"]).dt.strftime("%Y-%m-%d")
    keep_cols = ["Periode"] + list(COMPONENTS.values()) + ["Voix_SMS_Total_TTC", "Voix_SMS_Total_HT"]
    detail_display = detail_display[keep_cols].rename(columns={
        "Voix_TTC": "Voix TTC",
        "SMS_TTC": "SMS TTC",
        "Djiguiya_Voix_SMS_TTC": "Djiguiya Voix&SMS TTC",
        "Sewa_TTC": "Sewa TTC",
        "Netaa_Voix_TTC": "Netaa Voix TTC",
        "Forfaits_Voix_Inter_TTC": "Forfaits Voix Inter TTC",
        "PAYG_National_TTC": "PAYG National TTC",
        "Voix_SMS_Total_TTC": "Voix&SMS Total TTC",
        "Voix_SMS_Total_HT": "Voix&SMS Total HT",
    })
    ...
    summary_colors = {
        "Periode": "#F7F0D0",
        "Voix&SMS Total HT": "#D8E8BF",
        "Voix&SMS Total TTC": "#D7EBFB",
    }
    ...
    detail_colors = {
        ...
        "Voix TTC": "#D8E8BF",
        "SMS TTC": "#D7EBFB",
        "Djiguiya Voix&SMS TTC": "#F2DCCD",
        "Sewa TTC": "#EFC9AB",
        "Netaa Voix TTC": "#B7CCE3",
        "Forfaits Voix Inter TTC": "#DCE8CC",
        "PAYG National TTC": "#CBD3DF",
        "Voix&SMS Total TTC": "#FFE6CC",
        "Voix&SMS Total HT": "#FFE6CC",
    }

    with st.expander("🔽 Tableau Synthèse Voix", expanded=False):
        render_html_table(summary_display, summary_colors, "VOIX & SMS - SYNTHESE")

    with st.expander("🔽 Tableau Détail Voix", expanded=False):
        render_html_table(detail_display, detail_colors, "VOIX & SMS - DETAIL")
    ...
    st.download_button(
        ...
        data=excel_bytes,
        file_name="voix_tableaux_colores.xlsx",
